...
impl BootInfoFrameAllocator {
    ...
    // valid. The main requirement is that all frames that are marked as `USABLE` in the memory map are
    // really unused.
...
unsafe impl FrameAllocator<Size4KiB> for BootInfoFrameAllocator {
    fn allocate_frame(&mut self) -> Option<PhysFrame> {
        ...
        self.next += 1; // this is the unsafe part
        frame
    }
}
...
pub fn create_example_mapping(
    ...
    let map_to_result = unsafe {
        // FIXME: this is not safe, we do it for testing
